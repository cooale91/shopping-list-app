...
def shop_url(store, query):
    return SUPERMARKETS[store] + urllib.parse.quote(query)
...
<div class="hero">
  <h1>Meal Planner</h1>
  <p>Choose recipes, add your own, and turn them into a shopping list built for phone and tablet screens.</p>
</div>
""", unsafe_allow_html=True)

tab_plan, tab_shop, tab_add = st.tabs(["Plan", "Shop", "Add recipe"])
...
with tab_shop:
    ...
    with c2:
        st.caption("Search links match your chosen store")
    ...
    if not selected:
        ...
        st.text_area("Bulk list to copy", value=plain_bulk, height=140, key="plain_bulk")
        d1, d2 = st.columns(2)
        with d1:
...
with tab_add:
    ...
    st.caption("Add up to 8 ingredients in one go. New recipes save into recipes.json.")
...
<div class="bottom-hint">
  <strong>Mobile tip:</strong> use the <em>Shop</em> tab to copy the bulk list, then tap the item buttons to open supermarket searches.
</div>
""", unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)

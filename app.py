...
API_BASE_URL = os.getenv("MEAL_PLANNER_API_BASE_URL", "").rstrip("/")
API_KEY = os.getenv("MEAL_PLANNER_API_KEY", "")
USER_ID = os.getenv("MEAL_PLANNER_USER_ID", "demo-user")
...
.block-container {
    max-width: 560px; padding-top: 0.7rem; padding-bottom: 6.5rem;
    padding-left: 0.8rem; padding-right: 0.8rem;
}
header[data-testid="stHeader"] { background: rgba(255,255,255,0.0); }
section[data-testid="stSidebar"] { display: none; }
.app-shell {
    border: 1px solid rgba(80,80,80,0.20); border-radius: 24px; padding: 0.9rem;
    background: #ffffff; box-shadow: 0 8px 28px rgba(0,0,0,0.06);
}
.hero {
    padding: 1rem; border-radius: 20px; margin-bottom: 0.9rem;
    background: linear-gradient(135deg, rgba(0,120,255,0.12), rgba(0,180,120,0.08));
    border: 1px solid rgba(0,120,255,0.15);
}
.hero h1 { font-size: 1.55rem; margin: 0 0 0.2rem 0; color: #111111; }
.hero p { margin: 0; color: #2c2c2c; font-size: 1rem; line-height: 1.35; }
.section-card, .recipe-card {
    padding: 0.95rem; border-radius: 18px; margin-bottom: 0.85rem;
    border: 1px solid rgba(80,80,80,0.18); background: #ffffff;
}
.recipe-card { border-radius: 16px; background: #fcfcfc; margin-bottom: 0.8rem; }
.metric-pill {
    padding: 0.6rem 0.85rem; border-radius: 999px; display: inline-block;
    margin-right: 0.45rem; margin-bottom: 0.45rem;
    border: 1px solid rgba(80,80,80,0.18); background: #f7f7f7;
    font-size: 0.98rem; color: #111111;
}
.item-row { padding: 0.85rem 0.1rem; border-bottom: 1px solid rgba(120,120,120,0.14); }
.bottom-hint {
    position: sticky; bottom: 0.55rem; margin-top: 1rem; padding: 0.95rem;
    border-radius: 16px; background: #f8f8f8; border: 1px solid rgba(80,80,80,0.15); color: #111111;
}
.small-muted { color: #333333; font-size: 0.95rem; }
.stButton > button, .stDownloadButton > button, .stLinkButton > a {
    width: 100%; min-height: 3.5rem; font-size: 1.05rem; font-weight: 600; border-radius: 16px;
}
div[data-testid="stNumberInput"] input, div[data-testid="stTextInput"] input, textarea, input {
    font-size: 1.02rem !important; color: #111111 !important;
}
label, .stMarkdown, p, li, span, div { color: #111111; }
div[data-testid="stCheckbox"] label p { font-size: 1.02rem !important; color: #111111 !important; }
div[data-testid="stTabs"] button { font-size: 1rem; padding-top: 0.8rem; padding-bottom: 0.8rem; color: #111111; }
...
def infer_category(name: str) -> str:
    ...
    if any(t in text for t in vegetable_terms): return "Vegetables"
    if any(t in text for t in meat_terms): return "Meat"
    if any(t in text for t in dairy_terms): return "Dairy"
    if any(t in text for t in bakery_terms): return "Bakery"
    if any(t in text for t in pasta_terms): return "Pasta & Rice"
    if any(t in text for t in tins_terms): return "Tins & Jars"
    return "Other"
...
def fetch_shared_shopping_list():
...
def remove_shared_item(item: str):
...
if "manual_shared_item" not in st.session_state:
    st.session_state.manual_shared_item = ""

st.markdown('<div class="app-shell">', unsafe_allow_html=True)
st.markdown("""
<div class="hero">
  <h1>Meal Planner</h1>
  <p>Choose recipes, add your own, import a HelloFresh recipe link, and share a shopping list with Alexa.</p>
</div>
""", unsafe_allow_html=True)

tab_plan, tab_shop, tab_shared, tab_add, tab_import = st.tabs(["Plan", "Shop", "Alexa list", "Add recipe", "Import URL"])
...
with tab_plan:
    ...
    for recipe in filtered:
        ...
        with c1:
            ...
            st.markdown(f'<div class="small-muted">{recipe["default_servings"]} servings · {len(recipe["ingredients"])} ingredients</div>', unsafe_allow_html=True)
        with c2:
            servings = st.number_input("Servings", min_value=1, max_value=20, value=int(st.session_state.servings.get(recipe["name"], recipe["default_servings"])), step=1, key=f"serv_{recipe['name']}")
...
with tab_shop:
    ...
    with c2:
        st.caption("Buttons sized for a Fire 7 screen")
    st.session_state.pantry_text = st.text_area("Pantry items to exclude", value=st.session_state.pantry_text, height=100, placeholder="salt\nolive oil\nblack pepper")
    ...
    if not selected:
        ...
        if backend_enabled():
            if st.button("Send all shopping items to Alexa list"):
                try:
                    sent = 0
                    for item in items:
                        add_shared_item(item["name"])
                        sent += 1
                    st.success(f"Sent {sent} items to the shared Alexa list.")
                except Exception as e:
                    st.error(f"Could not send items to the shared list: {e}")
        else:
            st.caption("Set backend environment variables to enable Alexa list sync.")
        ...
        for category, entries in grouped.items():
            ...
            for item in entries:
                ...
                a, b, c = st.columns([1.8, 0.9, 0.9])
                with a:
                    st.markdown(f'<div class="item-row"><strong>{item["name"]}</strong><br><span class="small-muted">{qty_text(item["quantity"], item["unit"])} · search: {query}</span></div>', unsafe_allow_html=True)
                with b:
                    st.link_button("Shop", link, use_container_width=True)
                with c:
                    if backend_enabled():
                        if st.button("Alexa", key=f"alexa_{category}_{item['name']}"):
                            try:
                                add_shared_item(item["name"])
                                st.success(f"Added {item['name']} to shared Alexa list.")
                            except Exception as e:
...
with tab_shared:
    st.markdown('<div class="section-card">', unsafe_allow_html=True)
    st.subheader("Shared Alexa shopping list")
    st.caption("This tab uses the same backend that the Alexa skill uses.")
    if not backend_enabled():
        st.warning("Backend not configured yet. Set MEAL_PLANNER_API_BASE_URL and MEAL_PLANNER_API_KEY in Streamlit Cloud.")
    else:
        top = st.columns([1, 1])
        with top[0]:
            if st.button("Refresh shared list"):
                try:
                    st.session_state.shared_items = fetch_shared_shopping_list()
                except Exception as e:
                    st.error(f"Could not fetch shared list: {e}")
        with top[1]:
            if st.button("Clear shared list"):
                try:
                    st.session_state.shared_items = clear_shared_list()
                    st.success("Shared list cleared.")
                except Exception as e:
                    st.error(f"Could not clear shared list: {e}")

        add_cols = st.columns([2, 1])
        with add_cols[0]:
            st.session_state.manual_shared_item = st.text_input("Add item to shared list", value=st.session_state.manual_shared_item, placeholder="milk")
        with add_cols[1]:
            if st.button("Add item"):
                if not st.session_state.manual_shared_item.strip():
                    st.error("Enter an item first.")
                else:
                    try:
                        st.session_state.shared_items = add_shared_item(st.session_state.manual_shared_item.strip())
                        st.session_state.manual_shared_item = ""
                        st.success("Added item to shared list.")
                    except Exception as e:
                        st.error(f"Could not add item: {e}")

        if not st.session_state.shared_items:
            try:
                st.session_state.shared_items = fetch_shared_shopping_list()
            except Exception:
                pass
        ...
        if not st.session_state.shared_items:
            st.info("The shared list is empty.")
        else:
            st.markdown(f'<span class="metric-pill">{len(st.session_state.shared_items)} shared items</span>', unsafe_allow_html=True)
            for item in st.session_state.shared_items:
                row = st.columns([2.2, 1])
                with row[0]:
                    st.markdown(f'<div class="item-row"><strong>{item}</strong></div>', unsafe_allow_html=True)
                with row[1]:
                    if st.button("Remove", key=f"remove_shared_{item}"):
                        try:
                            st.session_state.shared_items = remove_shared_item(item)
                            st.rerun()
                        except Exception as e:
            ...
            for ing_name, ing_qty, ing_unit, ing_cat in rows:
                if ing_name.strip():
                    if float(ing_qty) <= 0:
                        ...
                        st.stop()
                    ingredients.append({"name": ing_name.strip(), "quantity": float(ing_qty), "unit": ing_unit.strip(), "category": ing_cat})
            if not ingredients:
...
with tab_import:
    ...
    st.session_state.hf_url = st.text_input("HelloFresh recipe URL", value=st.session_state.hf_url, placeholder="https://www.hellofresh.co.uk/recipes/...")
...
<div class="bottom-hint">
  <strong>Alexa sync:</strong> once the backend is configured, Alexa and this app can add to the same shared shopping list.
</div>
""", unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)
